...
- Built-in bookmarks and output profiles.
- Built-in picker, rollout restart helper, and smart log tailing.
...
Save and reuse frequent context/namespace targets:

    bookmark add <name> <context>/<namespace>
    bookmark add <name> <context> <namespace>
    bookmark add <name> /<namespace>
    bookmark use <name>
    bookmark list
...
Examples:
    ...
    bookmark add dev docker-desktop/default
    bookmark add ops /kube-system
    bookmark use dev
    bookmark list
    b dev
...
trace is a short alias for showcmd.
...
Interactively pick objects by number:

    pick <resource>
    pick <resource> <namespace>
    pick <resource> [namespace] --run "<template>"
    pick <resource> [namespace] --copy

Examples:

    pick pods
    pick deployments kube-system
    pick pods --run "logs -f {1}"
    pick pods kube-system --run "describe pod {1} -n {ns}"
    pick pods --copy
    pick pods --copy --run "describe pod {name}"

Notes:
- Prints the selected object name to stdout.
- Picker prompt supports fuzzy filtering:
    - Type text to filter matches.
    - Type a number to choose.
    - Type /clear to reset filter.
- --run template placeholders:
    - {1} or {name}: selected object name
    - {ns}: effective namespace for pick
- --copy copies the selected object name to the system clipboard.
...
- !!, help, ns, ctx, use, switch, bookmark, b, alias, dryrun, showcmd, trace, pick, restart, tail
...
- Bookmarks are stored in workspace-local ./.kube-shell-bookmarks when possible.
- If current directory cannot be resolved, kube-shell falls back to home directory for bookmark storage.
